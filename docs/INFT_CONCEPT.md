...
- **Substrate: the Hermes Agent** (`hermes-agent.nousresearch.com`,
  `NousResearch/hermes-agent`, MIT). Underneath, this agent IS a full Hermes Agent — the
  self-improving agent by Nous Research: a real terminal interface, a closed learning
  loop that writes and improves its own skills, scheduled automations, subagents, and it
  lives where you do (CLI, Telegram, Discord, Slack, WhatsApp, Signal).
...
Because it knows it is Hermes underneath, any material from the Hermes ecosystem — its
docs, skills on the agentskills.io standard, plugins from the Hermes repo or community —
