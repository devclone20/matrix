#!/usr/bin/env bash
# matrix (MATRIX iNFT monorepo) — Hermes substrate setup. Installs the Hermes Agent
# (Nous Research, MIT) with its official installer, verifies this repo's wiring, and
# never uses sudo. Prints every command before it runs. Safe to re-run (idempotent).
...
say "── matrix · Hermes substrate setup ────────────────────────────"
...
NAME="$(python3 -c "import json;print(json.load(open('identity.json'))['marketplace_name'])" 2>/dev/null || echo 'MATRIX')"
say ""
